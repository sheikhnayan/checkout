...
@endif
    ...
    <div class="content-wrapper">
        ...
        @php
            ...
            $referralRows = (isset($referralRows) && $referralRows->isNotEmpty()) ? $referralRows : $data->map(function ($row) {
                        ...
                        <div class="tab-pane fade show active" id="tab-shopify-conversion" role="tabpanel">
                            ...
                            <div class="row {{ \App\Models\Setting::showConversionRateCard() ? 'row-cols-2 row-cols-xl-4' : 'row-cols-1 row-cols-md-3' }} g-2 g-md-3 mb-3 mb-md-4" id="shopifyMetricCardsRow">
                                ...
                                <div class="col">
                                    <div class="shopify-metric-card p-2 p-md-3 rounded-3 cursor-pointer active" data-metric="sales" onclick="switchShopifyMetric('sales')">
                                        ...
                                        <div class="d-flex align-items-baseline justify-content-between flex-wrap gap-1 mt-1" style="min-width:0;">
                                            <span class="shopify-metric-val" id="shopifySalesVal">@if($isInitialShell ?? false)<span class="skeleton-shimmer" style="width:100px;height:24px;vertical-align:middle;"></span>@else${{ number_format($todayRevenue ?? 0, 2) }}@endif</span>
                                            @if(\App\Models\Setting::showMetricTrends())
                                ...
                                <div class="col">
                                    <div class="shopify-metric-card p-2 p-md-3 rounded-3 cursor-pointer" data-metric="orders" onclick="switchShopifyMetric('orders')">
                                        ...
                                        <div class="d-flex align-items-baseline justify-content-between flex-wrap gap-1 mt-1" style="min-width:0;">
                                            <span class="shopify-metric-val" id="shopifyOrdersVal">@if($isInitialShell ?? false)<span class="skeleton-shimmer" style="width:50px;height:24px;vertical-align:middle;"></span>@else{{ number_format($todayTxns ?? 0) }}@endif</span>
                                            @if(\App\Models\Setting::showMetricTrends())
                                        ...
                                        <div class="text-white-50 small mt-1 text-truncate shopify-metric-subtext" id="shopifyOrdersSubtext" style="font-size:0.7rem;">
                                            <span id="shopifyOrdersSubtextBase">Today's bookings</span> · <span class="text-white-50 fw-semibold" id="shopifyGuestsWrap"><span id="shopifyGuestsVal">@if($isInitialShell ?? false)<span class="skeleton-shimmer" style="width:25px;height:12px;vertical-align:middle;"></span>@else{{ number_format($todayGuests ?? 0) }}@endif</span> guests</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col">
                                    <div class="shopify-metric-card p-2 p-md-3 rounded-3 cursor-pointer" data-metric="sessions" onclick="switchShopifyMetric('sessions')">
                                        ...
                                        <div class="d-flex align-items-baseline justify-content-between flex-wrap gap-1 mt-1" style="min-width:0;">
                                            <span class="shopify-metric-val" id="shopifySessionsVal">@if($isInitialShell ?? false)<span class="skeleton-shimmer" style="width:65px;height:24px;vertical-align:middle;"></span>@else{{ number_format($todaySessions ?? 0) }}@endif</span>
                                            @if(\App\Models\Setting::showMetricTrends())
                                ...
                                <div class="col">
                                    <div class="shopify-metric-card p-2 p-md-3 rounded-3 cursor-pointer" data-metric="conversion" onclick="switchShopifyMetric('conversion')">
                                        ...
                                        <div class="d-flex align-items-baseline justify-content-between flex-wrap gap-1 mt-1" style="min-width:0;">
                                            ...
                                            @endphp
                                            <span class="shopify-metric-val" id="shopifyConversionVal">@if($isInitialShell ?? false)<span class="skeleton-shimmer" style="width:60px;height:24px;vertical-align:middle;"></span>@else{{ number_format($initConv, 2) }}%@endif</span>
                                            @if(\App\Models\Setting::showMetricTrends())
                            ...
                            <div class="shopify-chart-wrap pt-3 border-top border-secondary border-opacity-25">
                                ...
                                <div style="height: 220px; position: relative;">
                                    @if($isInitialShell ?? false)
                                    <div id="chartSkeletonOverlay" class="position-absolute top-0 start-0 w-100 h-100 d-flex flex-column align-items-center justify-content-center rounded-3" style="background: rgba(15, 23, 42, 0.75); backdrop-filter: blur(4px); z-index: 5;">
                                        <div class="spinner-border spinner-border-sm text-purple mb-2" role="status" style="color: #c084fc; width: 1.5rem; height: 1.5rem;"></div>
                                        <div class="text-white-50 small fw-semibold">Loading analytics timeline...</div>
                                    </div>
                                    @endif
        ...
        <div class="txn-table-card mb-5">
            ...
            @if($isInitialShell ?? false)
            <div id="txnLoadingNotice" class="d-flex align-items-center justify-content-between p-2.5 px-3 mb-3 rounded-3" style="background:rgba(139,92,246,0.12);border:1px solid rgba(139,92,246,0.25);color:#c084fc;font-size:0.82rem;">
                <div class="d-flex align-items-center gap-2">
                    <div class="spinner-border spinner-border-sm" role="status" style="width:1rem;height:1rem;"></div>
                    <span class="fw-semibold">Loading transactions and reservations...</span>
                </div>
                <span class="text-white-50 small d-none d-md-inline">Populating live records and filters</span>
            </div>
            @endif
            ...
            <div class="table-responsive">
                <table class="txn-table w-100" id="txnDataTable">
                    ...
                    <tbody>
                        @if($isInitialShell ?? false)
                            @include('admin.transaction.partials.skeleton_rows')
                        @else
                            @include('admin.transaction.partials.table_rows', [
                                'data' => $data,
                                'canArchiveTransactions' => $canArchiveTransactions ?? false,
                                'isArchivedView' => $isArchivedView ?? false,
                                'isPayoutPage' => $isPayoutPage ?? false,
                            ])
                        @endif
                    </tbody>
                    <tfoot>
...
<!-- DataTables JS -->
            ...
            $(document).ready(function() {
                ...
                window.__isInitialShell = {{ ($isInitialShell ?? false) ? 'true' : 'false' }};
                let table = null;

                function initTxnDataTable() {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response && response.html) {
                                ...
                                $('#txnLoadingNotice').fadeOut(250, function() { $(this).remove(); });
                        ...
                        error: function(xhr) {
                            console.error('Failed to load transaction data async:', xhr);
                            $('#txnLoadingNotice').html('<span class="text-danger"><i class="fas fa-exclamation-triangle me-2"></i>Failed to load live data. <a href="javascript:location.reload()" class="text-white text-decoration-underline ms-2">Retry</a></span>');
                            $('#chartSkeletonOverlay').fadeOut(250);
                        }
                    });
                }

                if (window.__isInitialShell) {
