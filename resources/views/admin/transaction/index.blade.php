                ...
                <table class="txn-table w-100" id="txnDataTable">
                    ...
                    <tbody>
                        @forelse($data as $item)
                            @include('admin.transaction.partials.table_row', ['item' => $item, 'isArchivedView' => $isArchivedView, 'canArchiveTransactions' => $canArchiveTransactions, 'isPayoutPage' => $isPayoutPage ?? false])
                        @empty
                        @endforelse
                    </tbody>
                    <tfoot>
...
<!-- DataTables JS -->
            ...
            $(document).ready(function() {
                ...
                let table = $('#txnDataTable').DataTable({
                    ...
                    serverSide: true,
                    ajax: {
                        url: "{{ route('admin.transaction.ajax-data') }}",
                        type: 'GET',
                        data: function(d) {
                            d.search_query = $('#txnSearch, #mobileTxnSearch').val() || '';
                            d.date_range = String($('#txnDateRange').val() || $('#mobileTxnDateRange').val() || '').trim();
                            d.date_target = String($('#dateTargetSelect').val() || $('#mobileDateTargetSelect').val() || 'either').toLowerCase();
                            ...
                            d.host = $('.polaris-filter-cb[data-category="host"]:checked').map(function() { return $(this).val(); }).get();
                            d.venue = $('.polaris-filter-cb[data-category="venue"]:checked').map(function() { return $(this).val(); }).get();
                            d.type = "{{ request('type') }}";
                            d.website = "{{ request('website') }}";
                            d.archived = "{{ request('archived') }}";
                        },
                        dataSrc: function(json) {
                            if (json && json.analytics) {
                                updateShopifyAnalyticsFromJsonObject(json.analytics);
                            }
                            return json.data || [];
                        }
                    },
                ...
                window.latestAnalyticsData = null;
                ...
                $(document).on('shown.bs.tab', 'button[data-bs-toggle="pill"]', function(e) {
                    ...
                    if (window.latestAnalyticsData) {
                        updateShopifyAnalyticsFromJsonObject(window.latestAnalyticsData);
                    }
                });
                ...
                function updateShopifyAnalyticsFromJsonObject(analytics) {
                    if (!analytics) return;
                    window.latestAnalyticsData = analytics;

                    const totalSales = parseFloat(analytics.total_sales || 0);
                    const totalOrders = parseInt(analytics.total_orders || 0, 10);
                    const dailyMap = analytics.daily_map || {};
                    ...
                    // Calculate period-over-period deltas (% comparisons)
                    let salesDelta = 0;
                    let ordersDelta = 0;
                    let sessionsDelta = 0;
                    let convDelta = 0;
                ...
                $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
                    if (settings.nTable.id !== 'txnDataTable' || settings.oFeatures.bServerSide) {
                        return true;
                    }
