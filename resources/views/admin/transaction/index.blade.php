            ...
            $(document).ready(function() {
                ...
                $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
                    ...
                    if (activeVenues.length > 0) {
                        const rowVenue = String($viewBtn.data('venue_name') || $viewBtn.data('website_id') || $row.find('.txn-pkg-name div').first().text() || '').trim().toLowerCase();
                        const matches = activeVenues.some(function(v) {
                            const sel = String(v || '').trim().toLowerCase();
                            if (!sel) return false;
                            return rowVenue === sel || rowVenue.includes(sel) || sel.includes(rowVenue);
                        });
