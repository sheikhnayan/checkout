...
<body>
    ...
    <div id="feed-copy-toast" role="status" aria-live="polite"><i class="fas fa-check me-1"></i>Link copied!</div>
        ...
        if (isIOSDevice && 'IntersectionObserver' in window) {
            var iosVideoObserver = new IntersectionObserver(function (entries) {
                entries.forEach(function (entry) {
                    ...
                    var v = entry.target;
                    if (v.closest('.feed-lightbox')) { return; }
                    if (v.paused) {
                        v.muted = true;
                        v.defaultMuted = true;
                        var p = v.play();
