                        ...
                        <tr>
                            ...
                            <td>
                                @if($linkedTransaction && $displayTransactionId && $hasPurchaseRef)
                                    {!! preg_replace(
                                        '/purchase\s*#\d+/i',
                                        'purchase <a href="#" class="wallet-transaction-link" data-table-id="affiliateWalletTransactionTable" data-transaction-row-id="' . e($linkedTransaction->id) . '">#' . e($displayTransactionId) . '</a>',
                                        e($descriptionText),
                                        1
                                    ) !!}
                                @elseif($linkedTransaction && $displayTransactionId)
                                    {{ $descriptionText }}
                                    <span class="ms-1">(<a href="#" class="wallet-transaction-link" data-table-id="affiliateWalletTransactionTable" data-transaction-row-id="{{ $linkedTransaction->id }}">#{{ $displayTransactionId }}</a>)</span>
                                @else
                                    {{ $item->description }}
                                @endif
                            </td>
