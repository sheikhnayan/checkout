...
# Header
cover_content.append(Paragraph('Bangladesh University of Business and Technology', header_style))
cover_content.append(Spacer(1, 0.35*inch))

# Logo
try:
    logo = Image('C:/tmp/bubt_logo/bubt_logo.png', width=1.1*inch, height=1.1*inch)
    logo_table = Table([[logo]], colWidths=[5.7*inch])
    logo_table.setStyle(TableStyle([('ALIGN', (0, 0), (-1, -1), 'CENTER'), ('VALIGN', (0, 0), (-1, -1), 'MIDDLE')]))
    cover_content.append(logo_table)
except:
    pass
...
# ASSIGNMENT Box
cover_content.append(Paragraph('ASSIGNMENT', assignment_style))
cover_content.append(Spacer(1, 0.25*inch))

# Course Info
cover_content.append(Paragraph('Assignment No :', field_label_style))
cover_content.append(Paragraph('Course Code : ENG 3102', field_label_style))
cover_content.append(Paragraph('Course Title : 18th Century English Literature', field_label_style))
cover_content.append(Spacer(1, 0.25*inch))

# Submitted By Box
submitted_by_left = []
submitted_by_left.append(Paragraph('Submitted By:', box_title_style))
submitted_by_left.append(Paragraph('Name : Koli Akter', box_text_style))
submitted_by_left.append(Paragraph('ID No : 056', box_text_style))
submitted_by_left.append(Paragraph('Intake : 59', box_text_style))
submitted_by_left.append(Paragraph('Section : 1', box_text_style))

# Submitted To Box
submitted_to_right = []
submitted_to_right.append(Paragraph('Submitted To:', box_title_style))
submitted_to_right.append(Paragraph('Name : Mahmud Sazzad', box_text_style))
submitted_to_right.append(Spacer(1, 0.05*inch))
submitted_to_right.append(Paragraph('Dept. of Bangladesh University of Business & Technology', box_text_style))

# Two boxes table
boxes = Table([
    [submitted_by_left, submitted_to_right]
], colWidths=[2.7*inch, 2.7*inch])

boxes.setStyle(TableStyle([
    ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
    ('VALIGN', (0, 0), (-1, -1), 'TOP'),
    ('BORDER', (0, 0), (0, 0), 1.5, colors.HexColor('#000000')),
    ('BORDER', (1, 0), (1, 0), 1.5, colors.HexColor('#000000')),
    ('LEFTPADDING', (0, 0), (-1, -1), 0.12*inch),
    ('RIGHTPADDING', (0, 0), (-1, -1), 0.12*inch),
    ('TOPPADDING', (0, 0), (-1, -1), 0.15*inch),
    ('BOTTOMPADDING', (0, 0), (-1, -1), 0.12*inch),
    ('ROWHEIGHTS', (0, 0), (-1, -1), 1.65*inch),
]))

cover_content.append(boxes)
cover_content.append(Spacer(1, 0.2*inch))

# Date of Submission
today = date.today().strftime('%d %B %Y')
cover_content.append(Paragraph(f'Date of Submission : {today}', field_label_style))
cover_content.append(Spacer(1, 0.1*inch))

# Create main cover table with THICK border
cover_table = Table([
    [cover_content]
], colWidths=[5.7*inch])
...
cover_table.setStyle(TableStyle([
    ...
    ('VALIGN', (0, 0), (-1, -1), 'TOP'),
    ('BORDER', (0, 0), (-1, -1), 2.5, colors.HexColor('#000000')),
    ('LEFTPADDING', (0, 0), (-1, -1), 0.35*inch),
    ('RIGHTPADDING', (0, 0), (-1, -1), 0.35*inch),
    ('TOPPADDING', (0, 0), (-1, -1), 0.3*inch),
    ('BOTTOMPADDING', (0, 0), (-1, -1), 0.3*inch),
]))
