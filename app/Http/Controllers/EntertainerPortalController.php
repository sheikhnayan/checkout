...
class EntertainerPortalController extends Controller
{
    ...
    public function updateSettings(Request $request)
    {
        ...
        $currentGalleryImages = array_values(array_filter((array) $entertainer->gallery_images));
        ...
        foreach ($this->normalizeImageFiles($request->file('gallery_images')) as $index => $image) {
            ...
            $name = 'entertainer_gallery_' . $entertainer->id . '_' . time() . '_' . $index . '_' . uniqid() . '.' . $image->getClientOriginalExtension();
